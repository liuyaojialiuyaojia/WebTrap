#!/usr/bin/env python3
"""Inject the official Combined Attack string into web page metadata."""
...
from web.baseline.combinedattack.build_payload import build_combinedattack_text
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--out-artifact", type=Path, default=None)
    try:
...
def _resolve_site_paths(base_root: Path, args: argparse.Namespace) -> None:
    ...
    args.out_artifact = resolve_path_within(
        args.out_artifact,
        root=static_root,
        relative="combinedattack_payload.json",
    )


def inject_combinedattack(argv: Optional[Sequence[str]] = None) -> dict[str, Any]:
    ...
    seed_material = f"{microtree_id}::{inject_page_index}"
    ...
    attack_id = f"combinedattack::official::{microtree_id}::{inject_page_index}"
    artifact = {
        "baseline_family": "combinedattack",
    ...
    _write_json(args.out_artifact, artifact)
    ...
    injection_entry: dict[str, Any] = {
        ...
        "source": {
            ...
            "payload_artifact": str(args.out_artifact),
        },
        "combinedattack": {
            "single_node": True,
            "official_variant_name": "completion_realcmb",
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    result = inject_combinedattack(argv)
