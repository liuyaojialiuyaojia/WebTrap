...
This copy lives under web/baseline/ so the baseline implementation stays isolated from web/psaa/.
"""
    ...
    function openPageSafe(index, cause) {
      ...
      if (targetPage) { targetPage.style.display = 'block'; try { targetPage.scrollIntoView({ block: 'start' }); } catch {} }
	      try {
	        ...
	        if (curHash.startsWith('#OBJECTIVE:')) {
	          // Preserve the injected OBJECTIVE hash across navigation (WASP url_injection-style).
	          // Record page index in the querystring instead so the URL still changes with navigation.
	          try {
	            const url = new URL(window.location.href);
	            url.searchParams.set('p', String(Number(index)));
            ...
            if (inj.show_breadcrumb && typeof page.breadcrumb === 'string') {
              const sm = document.createElement('small'); sm.className = 'trap-inject-breadcrumb text-muted'; sm.textContent = 'Current path ' + page.breadcrumb; blk.appendChild(sm);
	            ...
	            if (inj.cta && inj.cta.target_url) {
	              ...
	              a.href = inj.cta.target_url; a.textContent = (inj.cta.label || 'this link');
	              a.addEventListener('click', (ev) => {
	                ...
	                try {
	                  ...
	                  const i = raw.indexOf('#');
	                  hash = i >= 0 ? raw.slice(i) : '';
	                }
	                if (hash && hash.startsWith('#OBJECTIVE:')) {
	                  try { window.location.hash = hash; } catch {}
	                }
	                const status = document.getElementById('status');
	                if (status) status.textContent = 'Attack CTA clicked → objective injected into URL';
	              }, true);
	              blk.appendChild(a);
	            }
    ...
    function attachTrapFallbackHandlers(metadata) {
      ...
      if (!trapFallbackInstalled) {
        ...
        document.addEventListener('click', event => {
          ...
          if (!a) return;
          const status = document.getElementById('status'); if (status) status.textContent = 'Attack CTA clicked → objective injected into URL';
        }, true);
